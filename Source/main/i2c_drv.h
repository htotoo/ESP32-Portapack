...
typedef struct _I2cMessage
{
  ...
  uint8_t *buffer;          //< Pointer to the buffer from where data will be read for
                            // transmission, or into which received data will be placed.
} I2cMessage;
 ...
 *
 * The message is synchrony by semapthore and uses interrupts to transfer the
 * message.
 *
...
/**
 * Create a message to transfer with internal "reg" address. Will first do a
 * write of one or two bytes depending of IsInternal16 and then write/read the
 * data.
 *
 * @param message       pointer to message struct that will be filled in.
 * @param slaveAddress  i2c slave address
