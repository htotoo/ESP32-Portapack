...
TIR::TIR(gpio_num_t tx, gpio_num_t rx, uint32_t freq)
{
    tx_pin = tx;
    // init rx
    if (rx != 0)
    ...
    if (tx_pin != 0)
    {
        ...
        xTaskCreate(processSendTask, "processSendTask", 2048, NULL, 5, NULL);
    }
}
...
size_t TIR::rmt_encode_ir(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    if (ir_encoder->state == 1)
    {
        ...
        {
            if (send_data->protocol & (1 << (send_data->bits - 1 - i)))
            {
