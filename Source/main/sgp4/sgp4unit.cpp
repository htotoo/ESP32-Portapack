...
static void dscom(
    ...
    xnodce = floatmod(4.5236020 - 9.2422029e-4 * day, twopi);
    stem = sin(xnodce);
    ctem = cos(xnodce);
    ...
    zmol = floatmod(4.7199672 + 0.22997150 * day - gam, twopi);
    zmos = floatmod(6.2565837 + 0.017201977 * day, twopi);
...
static void dsinit(
    ...
    dndt = 0.0;
    theta = floatmod(gsto + tc * rptim, twopi);
    em = em + dedt * t;
    inclm = inclm + didt * t;
    argpm = argpm + domdt * t;
    nodem = nodem + dnodt * t;
    ...
    if (irez != 0) {
        ...
        if (irez == 2) {
            ...
            xlamo = floatmod(mo + nodeo + nodeo - theta - theta, twopi);
        ...
        if (irez == 1) {
            ...
            xlamo = floatmod(mo + nodeo + argpo - theta, twopi);
...
static void initl(
    ...
    if (opsmode == 'a') {
        ...
        c1p2p = c1 + twopi;
        gsto = floatmod(thgr70 + c1 * ds70 + c1p2p * tfrac + ts70 * ts70 * fk5r, twopi);
        if (gsto < 0.0)
            gsto = gsto + twopi;
    } else
...
bool sgp4init(
    ...
    if ((omeosq >= 0.0) || (satrec.no >= 0.0)) {
        ...
        satrec.cc4 = 2.0 * satrec.no * coef1 * ao * omeosq *
                     (satrec.eta * (2.0 + 0.5 * etasq) + satrec.ecco * (0.5 + 2.0 * etasq) - j2 * tsi / (ao * psisq) * (-3.0 * satrec.con41 * (1.0 - 2.0 * eeta + etasq * (1.5 - 0.5 * eeta)) + 0.75 * satrec.x1mth2 * (2.0 * etasq - eeta * (1.0 + etasq)) * cos(2.0 * satrec.argpo)));
        ...
        satrec.argpdot = -0.5 * temp1 * con42 + 0.0625 * temp2 * (7.0 - 114.0 * cosio2 + 395.0 * cosio4) +
...
bool sgp4(
    ...
    nodem = floatmod(nodem, twopi);
    argpm = floatmod(argpm, twopi);
    xlm = floatmod(xlm, twopi);
    mm = floatmod(xlm - argpm - nodem, twopi);
